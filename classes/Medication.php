...
class Medication {
    ...
    private $study_id, $patient_medications;
    ...
    public function __construct($module, $pid, $record_id, $study_id, $smaData, $fhirValues) {
        ...
        $this->instrument = $this->module->getProjectSetting('medication-form', $this->pid);
        $this->event_id = $this->module->getProjectSetting('medication-event', $this->pid);
        ...
        $this->med_list_pid = $this->module->getSystemSetting('medication-project');
        try {
            ...
            $this->module->emError("Could not retrieve the data dictionary for project $this->med_list_pid");
        }
    ...
    public function sendMedicationData() {
        ...
        foreach($medications as $record_id => $medicationInfo) {
            ...
            if (!$status) {
                $this->module->emError("Error sending data for project $this->pid, record $this->record_id, Medication " . json_encode($medicationInfo) . ". Error $error");
            } else {

                // Set the checkbox to say the data was sent to CureSMA
                $this->module->emDebug("Successfully sent medication ID for project $this->pid, record $this->record_id, Medication " . json_encode($medicationInfo));
    ...
    /**
     * This function updates the Medication instrument when the data was transferred to CureSMA.
     *
     * @return void
     */
    private function updatePatientRecords() {
        ...
        foreach ($this->patient_medications[$this->record_id][$this->event_id] as $instance_id => $medInfo) {
            ...
            $this->module->emDebug("Data to save: " . json_encode($medInfo));
            try {
                $rf = new RepeatingForms($this->pid, $this->instrument);
                $status = $rf->saveInstance($this->record_id, $medInfo, $instance_id, $this->event_id);
                $this->module->emDebug("Status from update medication list ID: $status for record $this->record_id instance $instance_id");
                if (!$status) {
                    $this->module->emError("Unable to save med_list_id for record $this->record_id, instance $instance_id with status $status");
                }
            } catch (Exception $ex) {
                $this->module->emError("Exception when instantiating the Repeating Forms class for project $this->pid instrument $this->instrument to save instance $instance_id");
            ...
            foreach ($this->patient_medications[$this->record_id][$this->event_id] as $instance_id => $medInfo) {
                ...
                if (empty($already_submitted_meds[$medInfo['med_snomed_ct_code']])) {
                    ...
                    if (!in_array($medInfo['med_snomed_ct_code'], $snomed_ids_to_be_added)) {
                        ...
                        //$this->module->emDebug("snomeds to be added: " . json_encode($data_to_save));
                    }
                }
            }
    ...
    private function saveMedicationStatus($record_id, $medicationInfo) {
        ...
        if (!empty($return['errors'])) {
            $this->module->emError("Could not save data for record $record_id, project $this->med_list_pid");
        } else {
            $this->module->emDebug("Successfully added new medication for record $record_id, project $this->med_list_pid");
        }
    }
